...
const RESTORE_INSTRUCTIONS_ABS_PATH = `${AlacranConstants.restoreDirectoryPath}/${RESTORE_INSTRUCTIONS}`

export interface IBackupCallbacks {
    getNodesInfo: () => Promise<ServerDockerInfo[]>
    getAlacranSalt: () => string
    getCertbotManager: () => CertbotManager
}

const BACKUP_META_DATA_ABS_PATH = `${AlacranConstants.restoreDirectoryPath}/meta/${BACKUP_JSON}`
export default class BackupManager {
    ...
    startRestorationIfNeededPhase1(alacranIpAddress: string) {
        // if (/alacran/restore/restore-instructions.json does exist):
        ...
        // - Create a alacran-salt secret using the data in restore
        ...
        return Promise.resolve()
            ...
            .then(function (nodesInfo) {
                ...
                const configFilePathRestoring =
                    AlacranConstants.restoreDirectoryPath +
                    '/data/config-alacran.json'
                const configData: {
            ...
            .then(function () {
                return fs.move(
                    AlacranConstants.restoreDirectoryPath + '/data',
                    AlacranConstants.alacranDataDirectory,
                    { overwrite: true }
                )
            })
            .then(function () {
    ...
    startRestorationIfNeededPhase2(
        ...
        // if (/alacran/restore/restore.json exists) GO TO RESTORE MODE:
        // - Double check salt against "meta/alacran-salt"
        // - Iterate over all APPs and make sure they are inited properly
        // - Delete /alacran/restore
            ...
            .then(function () {
                ...
                return Promise.resolve() //
                    ...
                    .then(function (data: BackupMeta) {
                        ...
                        if (restoringSalt !== alacranSalt) {
                            throw new Error(
                                `Salt does not match the restoration data: ${alacranSalt} vs  ${restoringSalt}`
                            )
                        }
                    ...
                    .then(function () {
                        return fs.remove(AlacranConstants.restoreDirectoryPath)
                    })
            })
    }
    ...
    checkAndPrepareRestoration() {
        ...
        return Promise.resolve() //
            .then(function () {
                ...
                if (fs.pathExistsSync(AlacranConstants.restoreTarFilePath)) {
                    Logger.d(
     ...
     *
     * /alacran/restore/restore-instructions.json
     *
     */
    private createRestorationInstructionFile() {
        ...
        return Promise.resolve() //
            .then(function () {
                const dirPath = AlacranConstants.restoreDirectoryPath
                ...
                if (fs.pathExistsSync(RESTORE_INSTRUCTIONS_ABS_PATH)) {
                    throw new Error(
                        'Restore instruction already exists! Cleanup your /alacran directory and start over.'
                    )
                }
        ...
        oldServers.forEach((s) => {
            if (s.isLeader) {
                ...
                ret.nodesMapping.push({
                    ...
                    oldIp: s.ip,
                    privateKeyPath:
                        AlacranConstants.alacranBaseDirectory + '/id_rsa',
                    user: 'root',
                })
            }
    ...
    private extractBackupContentAndRemoveTar() {
        if (!fs.statSync(AlacranConstants.restoreTarFilePath).isFile())
            throw new Error('restore tar file is not a file!!')

        return Promise.resolve() //
            .then(function () {
                return fs.ensureDir(AlacranConstants.restoreDirectoryPath)
            })
            .then(function () {
                return tar
                    .extract({
                        file: AlacranConstants.restoreTarFilePath,
                        cwd: AlacranConstants.restoreDirectoryPath,
                    })
                    .then(function () {
                        return fs.remove(AlacranConstants.restoreTarFilePath)
                    })
                    .then(function () {
    ...
    private createBackupInternal(iBackupCallbacks: IBackupCallbacks) {
        ...
        return Promise.resolve() //
            .then(function () {
                ...
                // Check if exist /alacran/temp/backup, delete directory
                // Create directory /alacran/temp/backup/raw
                // Copy /alacran/data to .../backup/raw/data
                // Ensure .../backup/raw/meta/backup.json
                // Create tar file FROM: .../backup/raw/   TO: .../backup/backup.tar

                const RAW = AlacranConstants.alacranRootDirectoryBackup + '/raw'
                ...
                return Promise.resolve() //
                    ...
                    .then(function () {
                        ...
                        return new Promise(function (resolve, reject) {
                            const child = exec(
                                `mkdir -p {dest} && cp -rp  ${AlacranConstants.alacranDataDirectory} ${dest}`
                            )
                    ...
                    .then(function (nodes) {
                        ...
                        return self.saveMetaFile(`${RAW}/meta/${BACKUP_JSON}`, {
                            salt: iBackupCallbacks.getAlacranSalt(),
                            nodes: nodes,
                        })
                    })
                    .then(function () {
                        const tarFilePath =
                            AlacranConstants.alacranRootDirectoryBackup +
                            '/backup.tar'
                    ...
                    .then(function (tarFilePath) {
                        const namespace = AlacranConstants.rootNameSpace
                        let mainIP = ''
                        ...
                        const now = moment()
                        const newName = `${
                            AlacranConstants.alacranDownloadsDirectory
                        }/${namespace}/alacrity-backup-${`${now.format(
                            'YYYY_MM_DD-HH_mm_ss'
    ...
    deleteBackupDirectoryIfExists() {
        return Promise.resolve() //
            .then(function () {
                if (
                    fs.existsSync(AlacranConstants.alacranRootDirectoryBackup)
                ) {
                    return fs.remove(
                        AlacranConstants.alacranRootDirectoryBackup
                    )
