...
const builtInThemes = [] as AlaCrityTheme[]

/**
 * Parses a string containing themed configuration fields into a JSON object.
 * Each field must start with "###AlaCrityTheme." followed by the field name and its content.
...
function parseAlaCrityTheme(input: string) {
    const result = {} as { [id: string]: string }
    const lines = input.split('\n')
    let currentField = undefined as string | undefined

    lines.forEach((line, index) => {
        if (line.startsWith('###AlaCrityTheme.')) {
            // Calculate the start position for the field name and remove the prefix '###AlaCrityTheme.'
...
export class ThemeManager {
    ...
    deleteTheme(themeName: string) {
        ...
        return Promise.resolve() //
            ...
            .then(function ([themesFetched, currentTheme]) {
                ...
                const newThemes = [] as AlaCrityTheme[]
                themes.forEach((it) => {
    ...
    updateTheme(oldName: string, theme: AlaCrityTheme) {
        const self = this
        return Promise.resolve()
            .then(function () {
                theme.builtIn = false
    ...
    getCurrentTheme(): Promise<AlaCrityTheme | undefined> {
        const self = this
...
export class ThemeManagerPublic {
    private static themeManagerPublic = new ThemeManager(
        DataStoreProvider.getDataStore(AlacranConstants.rootNameSpace)
    )
