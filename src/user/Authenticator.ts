...
const alacranDefaultPassword = EnvVar.DEFAULT_PASSWORD ?? 'alacran42'
...
class Authenticator {
    ...
    constructor(secret: string, namespace: string) {
        ...
        this.tokenVersion = AlacranConstants.isDebug
            ? 'test'
    ...
    isPasswordCorrect(password: string, savedHashedPassword: string) {
        ...
        return Promise.resolve().then(function () {
            ...
            if (!savedHashedPassword) {
                return alacranDefaultPassword === password
            }
    ...
    static getAuthenticator(namespace: string): Authenticator {
        ...
        if (!authenticatorCache[namespace]) {
            const alacranSalt = Authenticator.mainSalt
            if (alacranSalt) {
