...
import { AlacranError } from './AlacranError'
import BaseApi from './BaseApi'

class ApiStatusCodes {
    static createError(code: number, message: string) {
        return new AlacranError(code, message || 'NONE')
    }

    static createCatcher(res: Response) {
        return function (error: AlacranError | any) {
            if (!error || error.errorStatus !== 404) {
                Logger.e(error)
            }

            if (error && error.alacranErrorType) {
                res.send(new BaseApi(error.alacranErrorType, error.apiMessage))
                return
            }
    ...
    static readonly STATUS_ERROR_ALACRAN_NOT_INITIALIZED = 1001
