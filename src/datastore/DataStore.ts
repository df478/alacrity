...
import AlacranEncryptor from '../utils/Encryptor'
...
const NGINX_ALACRAN_CONFIG = 'nginxAlacranConfig'
...
const DEFAULT_ALACRAN_ROOT_DOMAIN = 'alacran.localhost'

const DEFAULT_NGINX_BASE_CONFIG = fs
    .readFileSync(__dirname + '/../../template/base-nginx-conf.ejs')
    .toString()
const DEFAULT_NGINX_ALACRAN_CONFIG = fs
...
const SERVER_BLOCK_CONF_OVERRIDE_PATH =
    AlacranConstants.alacranDataDirectory + '/server-block-conf-override.ejs'
...
class DataStore {
    private encryptor: AlacranEncryptor
    ...
    constructor(namespace: string) {
        const data = new Configstore(
            `alacran-store-${namespace}`, // This value seems to be unused
            {},
            {
                configPath: `${AlacranConstants.alacranDataDirectory}/config-${namespace}.json`,
            }
        )
    ...
    setEncryptionSalt(salt: string) {
        this.encryptor = new AlacranEncryptor(this.namespace + salt)
    ...
    getThemes(): Promise<AlaCrityTheme[]> {
        const self = this
        return Promise.resolve().then(function () {
    ...
    saveThemes(themes: AlaCrityTheme[]) {
        const self = this
        return Promise.resolve().then(function () {
            self.data.set(
                THEMES,
    ...
    getRootDomain() {
        return this.data.get(CUSTOM_DOMAIN) || DEFAULT_ALACRAN_ROOT_DOMAIN
    }
    ...
    getNginxConfig() {
        ...
        return Promise.resolve().then(function () {
            return {
                ...
                alacranConfig: {
                    byDefault: DEFAULT_NGINX_ALACRAN_CONFIG,
                    customValue: self.data.get(NGINX_ALACRAN_CONFIG),
                },
            }
        })
    }
    ...
    setNginxConfig(baseConfig: string, alacranConfig: string) {
        ...
        return Promise.resolve().then(function () {
            ...
            self.data.set(NGINX_ALACRAN_CONFIG, alacranConfig)
        })
    }
