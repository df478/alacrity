...
router.post('/register/', function (req, res, next) {
    ...
    return Promise.resolve()
        ...
        .then(function () {
            const alacranDefinitionContent: IAlacranDefinition = {
                schemaVersion: 2,
                imageName: AlacranConstants.configs.appPlaceholderImageName,
            }

            const promiseToIgnore = serviceManager
                .scheduleDeployNewVersion(appName, {
                    alacranDefinitionContentSource: {
                        alacranDefinitionContent: JSON.stringify(
...
router.post('/update/', function (req, res, next) {
    ...
    const alacranDefinitionRelativeFilePath =
        req.body.alacranDefinitionRelativeFilePath
    ...
    if (
        ...
        !AlacranConstants.configs.disableEncryptedCheck
    ) {
        res.send(
            new BaseApi(
                ApiStatusCodes.ILLEGAL_PARAMETER,
