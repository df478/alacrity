...
router.post('/enableregistry/', function (req, res, next) {
    const alacranManager = AlacranManager.get()
    const password = uuid()
    ...
    return Promise.resolve()
        .then(function () {
            return AlacranManager.get().getDockerRegistry().enableRegistrySsl()
        })
        .then(function () {
            return alacranManager
                .getDockerRegistry()
        ...
        .then(function (allRegs) {
            ...
            const user = AlacranConstants.alacranRegistryUsername
            const domain = alacranManager
                .getDockerRegistry()
...
router.post('/disableregistry/', function (req, res, next) {
    ...
    return Promise.resolve()
        ...
        .then(function () {
            return alacranManager.getDockerRegistry().ensureServiceRemoved()
        })
        .then(function () {
