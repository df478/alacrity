...
router.post('/createbackup/', function (req, res, next) {
    const backupManager = AlacranManager.get().getBackupManager()

    Promise.resolve()
        .then(function () {
            return backupManager.createBackup(AlacranManager.get())
        })
...
router.post('/changerootdomain/', function (req, res, next) {
    ...
    AlacranManager.get()
        .changeAlacranRootDomain(requestedCustomDomain, !!req.body.force)
        .then(function () {
            res.send(
...
router.post('/enablessl/', function (req, res, next) {
    ...
    AlacranManager.get()
        ...
        .then(function () {
            // This is necessary as the CLI immediately tries to connect to https://alacran.root.com
    ...
    return Promise.resolve()
        ...
        .then(function (hasRootSsl) {
            return {
                ...
                forceSsl: AlacranManager.get().getForceSslValue(),
                rootDomain: dataStore.hasCustomDomain()
                    ? dataStore.getRootDomain()
                    : '',
                alacranSubDomain: AlacranConstants.configs.alacranSubDomain,
            }
        })
        .then(function (data) {
            const baseApi = new BaseApi(
                ApiStatusCodes.STATUS_OK,
                'Alacran info retrieved'
            )
            baseApi.data = data
            res.send(baseApi)
        })
...
router.get('/loadbalancerinfo/', function (req, res, next) {
    return Promise.resolve()
        .then(function () {
            return AlacranManager.get().getLoadBalanceManager().getInfo()
        })
        .then(function (data) {
            const baseApi = new BaseApi(
                ApiStatusCodes.STATUS_OK,
...
router.get('/diskcleanup/', function (req, res, next) {
    return Promise.resolve()
        .then(function () {
            return AlacranManager.get().getDiskCleanupManager().getConfigs()
        })
        .then(function (data) {
            const baseApi = new BaseApi(
                ApiStatusCodes.STATUS_OK,
...
router.post('/netdata/', function (req, res, next) {
    ...
    return Promise.resolve()
        .then(function () {
            return AlacranManager.get().updateNetDataInfo(netDataInfo)
        })
        .then(function () {
            const baseApi = new BaseApi(
                ApiStatusCodes.STATUS_OK,
...
router.post('/goaccess/', function (req, res, next) {
    ...
    return Promise.resolve()
        .then(function () {
            return AlacranManager.get().updateGoAccessInfo(goAccessInfo)
        })
        .then(function () {
            const baseApi = new BaseApi(
                ApiStatusCodes.STATUS_OK,
...
router.get('/goaccess/:appName/files', async function (req, res, next) {
    ...
    const loadBalanceManager = AlacranManager.get().getLoadBalanceManager()
...
router.get('/goaccess/:appName/files/:file', async function (req, res, next) {
    ...
    const { domainName, fileName } = AlacranManager.get()
    ...
    if (fileName.includes('Live')) {
        ...
        await DockerApi.get().createContainer({
            imageName: AlacranConstants.configs.goAccessImageName,
            command: ['./catchupLog.sh'],
            volumes: [
                {
                    hostPath: AlacranConstants.nginxSharedLogsPathOnHost,
                    containerPath: AlacranConstants.nginxSharedLogsPath,
                    mode: 'rw',
                },
            ],
            network: AlacranConstants.alacranNetworkName,
    ...
    res.sendFile(
        path,
        { root: AlacranConstants.nginxSharedLogsPathOnHost },
        function (error) {
            if (error !== undefined) {
...
router.get('/nginxconfig/', function (req, res, next) {
    return Promise.resolve()
        .then(function () {
            return AlacranManager.get().getNginxConfig()
        })
        .then(function (data) {
            const baseApi = new BaseApi(
                ApiStatusCodes.STATUS_OK,
...
router.post('/nginxconfig/', function (req, res, next) {
    ...
    const alacranConfigCustomValue = req.body.alacranConfig.customValue

    return Promise.resolve()
        .then(function () {
            return AlacranManager.get().setNginxConfig(
                baseConfigCustomValue,
                alacranConfigCustomValue
            )
        })
        .then(function () {
            const baseApi = new BaseApi(
...
router.get('/nodes/', function (req, res, next) {
    return Promise.resolve()
        .then(function () {
            return AlacranManager.get().getNodesInfo()
        })
        .then(function (data) {
            const baseApi = new BaseApi(
                ApiStatusCodes.STATUS_OK,
...
router.post('/nodes/', function (req, res, next) {
    ...
    const alacranIpAddress = req.body.alacranIpAddress
    const sshPort = parseInt(req.body.sshPort) || 22
    const sshUser = (req.body.sshUser || 'root').trim()

    if (!alacranIpAddress || !remoteNodeIpAddress || !privateKey) {
        res.send(
            new BaseApi(
                ApiStatusCodes.STATUS_ERROR_GENERIC,
                'Private Key, Alacran IP address, remote IP address and remote username should all be present'
            )
        )
        return
    }
