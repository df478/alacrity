...
import { AlacranError } from '../models/OtherTypes'
...
export function injectGlobal() {
    return function (req: Request, res: Response, next: NextFunction) {
        ...
        locals.namespace =
            req.header(AlacranConstants.headerNamespace) ||
            AlacranConstants.rootNameSpace

        if (locals.namespace !== AlacranConstants.rootNameSpace) {
        ...
        locals.initialized = AlacranManager.get().isInitialized()
        locals.forceSsl = AlacranManager.get().getForceSslValue()
...
export function injectUser() {
    return function (req: Request, res: Response, next: NextFunction) {
        ...
        Authenticator.getAuthenticator(namespace)
            .decodeAuthToken(req.header(AlacranConstants.headerAuth) || '')
            ...
            .catch(function (error: AlacranError) {
                if (error && error.alacranErrorType) {
                    res.send(
                        new BaseApi(error.alacranErrorType, error.apiMessage)
                    )
                    return
                }
                Logger.e(error)
...
export function injectUserForBuildTrigger() {
    return function (req: Request, res: Response, next: NextFunction) {
        ...
        const token = req.header(AlacranConstants.headerAppToken) as string
        const namespace = locals.namespace
        const appName = req.params.appName as string

        if (req.header(AlacranConstants.headerAuth)) {
...
export function injectUserUsingCookieDataOnly() {
    return function (req: Request, res: Response, next: NextFunction) {
        Authenticator.getAuthenticator(AlacranConstants.rootNameSpace)
            .decodeAuthTokenFromCookies(
                req.cookies[AlacranConstants.headerCookieAuth]
            )
            .then(function (user) {
                res.locals.user = user
