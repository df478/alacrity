...
class DockerApi {
    ...
    createJoinCommand(
        ...
        return `docker swarm join --token ${token} ${alacranIpAddress}:2377 --advertise-addr ${workerIp}:2377`
    }
                ...
                return self.dockerode.createContainer({
                    ...
                    HostConfig: {
                        ...
                        LogConfig: {
                            ...
                            Config: {
                                'max-size':
                                    AlacranConstants.configs.defaultMaxLogSize,
                            },
                        },
                        ...(sticky
                            ? {
                            ...
                            : {}),
                        AutoRemove: !AlacranConstants.isDebug && !sticky,
                    },
                })
            })
            .then(function (data) {
        ...
        const dataToCreate: any = {
            ...
            TaskTemplate: {
                ...
                LogDriver: {
                    Name: 'json-file',
                    Options: {
                        'max-size': AlacranConstants.configs.defaultMaxLogSize,
                    },
                },
            },
            EndpointSpec: {
    ...
    ensureSecretOnService(serviceName: string, secretName: string) {
        ...
        return self.dockerode
            ...
            .then(function (secrets) {
                ...
                // alacran-me, it also returns alacran-me1 and etc if exist
     ...
     *        type: <defaults to bind>, can be volume or tmpfs (not supported yet through alacran)
     *    }
     * ]
     * @param networks
     * @param arrayOfEnvKeyAndValue:
    ...
    updateService(
        ...
        return self.dockerode
            ...
            .then(function (readData) {
                ...
                // - Even with "docker service update srv-alacran--name --image nginx:1 --force" it will still fail
...
const dockerApiAddressSplited = (EnvVars.ALACRAN_DOCKER_API || '').split(':')
const connectionParams: Docker.DockerOptions =
    dockerApiAddressSplited.length < 2
        ? {
              socketPath: AlacranConstants.dockerSocketPath,
          }
        : dockerApiAddressSplited.length === 2
          ? {
...
connectionParams.version = AlacranConstants.configs.dockerApiVersion
...
new Docker(lowVersionDocker)
    ...
    .then((data) => {
        ...
        const majorNeeded = Number(
            AlacranConstants.configs.dockerApiVersion
                .replace('v', '')
                .split('.')[0]
        )
